...
**🆕 2025年9月更新**: Excelテンプレートはサーバー上に固定配置され、CSVファイル2個のみのアップロードで済むようになりました。
...
- **🆕 CSV2個のみ**: Excelテンプレートのアップロードは不要になりました
...
- 使用頻度も高くないし、そんなに遅くもないし使用時に起動すればいっかーと定期リクエストは停止しました。
  
### 追記
- 自社環境が使えるようになったからそっちに移行したい。
- 自社Githubはリポジトリが作成できない？アカウント切り替えがめんどい。。。
